...
Login Autorizador Geral
    ...
    Executar com verificacao    Input Password    Falha ao preencher campo de senha (GERAL)    css=input[name='ctl00$bodyHolder$txtSenha']    ${SENHA_GERAL}
...
Login Autorizador Raiadrogasil
    ...
    Executar com verificacao    Input Password    Falha ao preencher campo de senha (RAIADROGASIL)    css=input[name='ctl00$bodyHolder$txtSenha']    ${LOGIN_RAIADROGASIL}
    Executar com verificacao    Click Button    Botão de login não foi clicado    id=ctl00_bodyHolder_btnEnviarSenha
    Executar com verificacao    Wait Until Element Is Visible    Não foi redirecionado pra página principal    id=btnCapSearchPharma    10s
Login Autorizador Panvel
    ...
    Executar com verificacao    Input Password    Falha ao preencher campo de usuário (PANVEL)    css=input[name='ctl00$bodyHolder$txtUsername']    ${LOGIN_PANVEL}
    Executar com verificacao    Input Password    Falha ao preencher campo de senha (PANVEL)    css=input[name='ctl00$bodyHolder$txtSenha']    ${SENHA_PANVEL}
    Executar com verificacao    Click Button    Botão de login não foi clicado    id=ctl00_bodyHolder_btnEnviarSenha
    Executar com verificacao    Wait Until Element Is Visible    Não foi redirecionado pra página principal    id=btnCapSearchPharma    10s
...
Programa da Industria - CPF
    ...
    Executar com verificacao    Input Password    Falha ao preencher CPF    id=cpfValue    ${CPF}
...
Convenio Empresa - Cartao
    ...
    IF    ${cpf_input_existe}
        ...
        Executar com verificacao    Input Password    Falha ao preencher CPF    css=input.form-control.cpf.input-search-user-normal    ${CPF}
